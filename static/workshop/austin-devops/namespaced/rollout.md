...
## Set replicas for `worker` deployment

.exercise[

- Scale our worker deployment to 3:
  ```bash
...
- Let's monitor what's going on by running the following in a new terminal:
  ```bash
  watch -n 1 kubectl get all --selector "app=worker"
  ```
...
*This would be more apparent if we had 10 replicas, so lets pretend:

- Why is our app a bit slower?

- Because `MaxUnavailable=25%`

  ... So the rollout terminated 3 replicas out of 10 available
...
- We start with 3 pods running for the `worker` deployment

- Current settings: MaxUnavailable=25% and MaxSurge=25%

- When we start the rollout:

  - one replica are taken down (as per MaxUnavailable=25%)
  - one is created (with the new version)

- Now we have 3 replicas up and running, and 1 being deployed
